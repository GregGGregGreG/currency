...
import org.baddev.currency.ui.core.converter.DateToLocalDateTimeConverter;
import org.baddev.currency.ui.core.model.grid.DateTimeResolution;
...
import static org.baddev.currency.ui.core.util.UIUtils.*;
...
public final class ExchangesView extends AbstractGridView<IExchangeOperation> {
    ...
    private final Iso4217CcyService   iso4217CcyService;
    private final ExchangeRateService rateService;

    public ExchangesView(ExchangerService exchangerService, Iso4217CcyService iso4217CcyService, ExchangeRateService rateService) {
        super(IExchangeOperation.class, P_ID, P_USER_ID);
        this.exchangerService = exchangerService;
        this.iso4217CcyService = iso4217CcyService;
    ...
    protected final void setup(Grid grid) {
        ...
        grid.getColumn(P_FROM_AMOUNT)
                .setRenderer(new HtmlRenderer(), new DoubleAmountToStringConverter());
        grid.getColumn(P_RATES_DATE)
                .setHeaderCaption("Rate's Date")
                .setRenderer(FormatUtils.dateRenderer(false), new DateToLocalDateConverter());
        ...
        setSortOrder(P_PERFORM_DATETIME, SortDirection.DESCENDING);
        setHidableColumns(true);
    }

    @Override
    protected Collection<? extends IExchangeOperation> getItems() {
        return exchangerService.findForUser(SecurityUtils.getIdentityUserPrincipal().getId());
    }
    ...
    protected void customizeGridBar(HorizontalLayout gridBar) {
        ...
        gridBar.addComponents(exchDateF, amountF, fromCcyChoiseF, toCcyChoiseF, space, exchBtn, resetBtn);
        gridBar.setExpandRatio(space, 1.0f);
        gridBar.setComponentAlignment(exchDateF, Alignment.MIDDLE_LEFT);
        ...
        gridBar.setComponentAlignment(exchBtn, Alignment.BOTTOM_RIGHT);
