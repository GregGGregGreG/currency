...
(
  user_id            BIGINT(20) PRIMARY KEY             NOT NULL,
  ui_notifications   TINYINT(1) DEFAULT '1'             NOT NULL,
  mail_notifications TINYINT(1) DEFAULT '0'             NOT NULL,
  theme_name         VARCHAR(30) DEFAULT 'valo-default' NOT NULL,
  two_factor_auth    TINYINT(1) DEFAULT '0'             NOT NULL,
  email_sign_in      TINYINT(1) DEFAULT '1'             NOT NULL,
  CONSTRAINT user_preferences_user_id_fk FOREIGN KEY (user_id) REFERENCES user (id)
    ON DELETE CASCADE
);
CREATE UNIQUE INDEX user_preferences_user_id_uindex ON user_preferences (user_id);
...
CREATE INDEX user_password_history_user_id_fk ON user_password_history (user_id);
...
(
  ...
  CONSTRAINT password_reset_token_user_id_fk FOREIGN KEY (user_id) REFERENCES user (id)
    ON DELETE CASCADE
);
